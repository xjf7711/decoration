...
const START_EVENT: BaseEvent = { type: 'start' };
...
export default class OrbitControls extends THREE.EventDispatcher {
  ...
  constructor(
    ...
    this.onMouseDown = ((event: ThreeEvent) => {
      ...
      if (this.state !== STATE.NONE) {
        ...
        this.dispatchEvent(START_EVENT as never);
      }
    }) as EventListener;
    ...
    this.onTouchMove = ((event: ThreeEvent) => {
      ...
      event.preventDefault();
      event.stopPropagation();
      let element: HTMLElement | HTMLDocument;
      let dx: number;
      let dy: number;
      let distance: number;
      switch (event.touches.length) {
    ...
    this.onTouchEnd = () => {
      ...
      this.dispatchEvent(END_EVENT as never);
      this.state = STATE.NONE;
    };
  ...
  public update() {
    ...
    if (
      ...
      this.dispatchEvent(CHANGE_EVENT as never);
  ...
  public pan(deltaX: number, deltaY: number) {
    ...
    if (this._checkPerspectiveCamera(this.object)) {
      ...
      this.panLeft(
        (2 * deltaX * targetDistance) / (element as HTMLElement).clientHeight,
        this.object.matrix
      );
      this.panUp(
        (2 * deltaY * targetDistance) / (element as HTMLElement).clientHeight,
        this.object.matrix
      );
   ...
   * THREE.PerspectiveCamera in the if branch where the check was performed.
   ...
   * If the check passes (returns true) the passed camera will have the type
   * THREE.OrthographicCamera in the if branch where the check was performed.
   * @param camera Object to be checked.
   */
